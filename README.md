...
```shell
./gradlew keepSecrets -PfileName=secrets.json [-PappSignatures=YourAppSignatureForDebug,YourAppSignatureForRelease] [-PobfuscationKey=YourObfuscationKey] [-PpackageName=YourPackageName]
```
...
> Remember to replace `secrets.json` with the actual name of your JSON file containing the secrets.
